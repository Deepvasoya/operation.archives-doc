...
@section('content')
  <div class="card table-wrapper">
    <div class="card-header border-0 d-flex align-items-center justify-content-between">
      <h3 class="mb-0">Usagers</h3>
      <a href="{{ route('usagers.create') }}" class="btn btn-primary btn-sm">
        <i class="icon-base bx bx-plus"></i> Nouveau
      </a>
    </div>
    ...
    <div class="card-body border-bottom">
      <form method="GET" action="{{ route('usagers.index') }}" id="filterForm">
        <div class="row g-3 mb-3">
          ...
          <div class="col-md-2">
            ...
            <input type="text" name="filter_telephone" class="form-control form-control-sm"
              placeholder="Rechercher par téléphone..." value="{{ request('filter_telephone') }}">
          </div>
          <div class="col-md-2 d-flex align-items-end gap-2">
            <button type="submit" class="btn btn-primary btn-sm">
              <i class="icon-base bx bx-search"></i> Filtrer
            </button>
            @if (request()->hasAny(['filter_nom', 'filter_cin', 'filter_email', 'filter_telephone']))
              <a href="{{ route('usagers.index') }}" class="btn btn-secondary btn-sm">
          ...
          @forelse($usagers as $usager)
            <tr>
              ...
              <td>
                ...
                <form action="{{ route('usagers.destroy', $usager->id) }}" method="POST" class="d-inline">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-sm btn-danger"
                    onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet usager?')" title="Supprimer">
                    <i class="icon-base bx bx-trash"></i>
                  </button>
                </form>
                <a href="{{ route('usagers.show', $usager->id) }}" class="btn btn-sm btn-primary" title="Opérations">
                  Opérations
                </a>
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="6" class="text-center py-4">
                <p class="text-muted mb-0">Aucun usager trouvé</p>
              </td>
            </tr>
          @endforelse
        </tbody>
    ...
    <div class="card-footer py-4">
      <div class="d-flex justify-content-between align-items-center">
        <div class="text-muted small">
          Affichage de {{ $usagers->firstItem() ?? 0 }} à {{ $usagers->lastItem() ?? 0 }}
          sur {{ $usagers->total() }} résultat(s)
        </div>
        <nav aria-label="Pagination">
          {{ $usagers->appends(request()->query())->links() }}
        </nav>
      </div>
    </div>
  </div>
